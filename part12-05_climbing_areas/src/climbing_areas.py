...
def lenroute(ca:ClimbingArea):
    return ca.routes()

def sort_by_number_of_routes(ca):
    return sorted(ca, key=lenroute)

def diff (ca:ClimbingArea):
    return ca.hardest_route().grade

def sort_by_most_difficult(ca):
    return sorted(ca, key=diff, reverse=True)